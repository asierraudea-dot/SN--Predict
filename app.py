...
# =============================================================================
st.set_page_config(
    page_title="SN Predict — Occidente de Antioquia",
...
with st.sidebar:
    st.markdown(
        ...
        "<div style='font-size:11px;color:#93C5FD;font-weight:500'>Occidente Ant. · v3.2</div>"
        "</div></div></div>",
        unsafe_allow_html=True,
    )
    st.write("")
    ...
    st.divider()
    st.markdown(
        "<p style='font-size:10px;color:#94A3B8;font-weight:600;letter-spacing:.06em'>FILTROS — Tabla de avances y KPIs</p>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        "SN Predict © 2026 · Centro Occidente Ant.</p>",
        unsafe_allow_html=True,
    )
...
TOUR = [
    ("🎓 Bienvenido a SN Predict",
     "Analiza 2.551 fichas históricas de 22 municipios del Occidente de Antioquia (2020-2025). "
...
# =============================================================================
    ...
    st.markdown('<div class="sd">Distribución histórica · Municipios con mayor demanda por nivel · Tendencia territorial</div>', unsafe_allow_html=True)
    ...
    with cf2:
        ord_mun = st.radio("Ordenar municipios por:",
                           ["Aprendices","Fichas","Prom/ficha"],
                           horizontal=True, key="ord_mun")
    with cf3:
        filt_sec = st.selectbox("Sector productivo",
                                ["Todos"] + SECTORES, key="dem_sec")

    nf_lbl = LN.get(nf, nf) if nf != "Todos" else "todos los niveles"
    st.markdown("---")

    # ── Fila superior: barras nivel + donut sector ─────────────────────────
    ...
    with c1d:
        ...
        fig_n = go.Figure(go.Bar(
            ...
            hovertemplate="<b>%{x}</b>: %{y:,} aprendices<extra></extra>"))
        fig_n.update_layout(
            title=dict(text=f"Aprendices por nivel — {nf_lbl}", font=dict(size=13)),
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            showlegend=False, height=260, margin=dict(t=40,b=10,l=10,r=10),
    ...
    with c2d:
        ...
        fig_s = px.pie(values=[x[1] for x in SD], names=[x[0] for x in SD],
            hole=0.55, title="Fichas por sector productivo",
            color_discrete_sequence=["#1E40AF","#0F6E56","#F59E0B","#7C3AED",
                "#DC2626","#64748B","#0891B2","#BE123C","#059669","#D97706","#374151","#9333EA"])
        fig_s.update_layout(height=260, margin=dict(t=40,b=10,l=10,r=10),
            paper_bgcolor="rgba(0,0,0,0)", title=dict(font=dict(size=13)),
            legend=dict(font=dict(size=10), x=0.78))
        st.plotly_chart(fig_s, use_container_width=True)

    st.markdown("---")

    # ── Municipios con mayor demanda por nivel ────────────────────────────
    col_mun, col_prog = st.columns([1.1, 0.9])

    with col_mun:
        st.markdown(f"**🗺️ Municipios con mayor demanda — {nf_lbl}**")
        st.caption("Filtra los filtros globales de Municipio y Sector de la barra lateral aplican aquí.")

        # Datos según nivel
        if nf == "Todos":
            mun_data = sorted(MUN_STATS.items(), key=lambda x: -x[1]["ap"])
            rows_mun = [{"mun": k, "ap": v["ap"], "fichas": v["fichas"],
                         "prom": v["prom"], "sector": v["sector"],
                         "tend": v.get("tend",0)} for k,v in mun_data]
        else:
            rows_mun = MUN_NIV_D.get(nf, [])
            rows_mun = [r for r in rows_mun]

        # Filtrar por municipio global si aplica
        if g_mun != "Todos":
            rows_mun = [r for r in rows_mun if r.get("mun","") == g_mun]

        # Filtrar por sector si aplica
        if filt_sec != "Todos":
            rows_mun = [r for r in rows_mun
                        if MUN_STATS.get(r.get("mun",""),{}).get("sector","") == filt_sec]

        # Ordenar
        ord_key = {"Aprendices":"ap","Fichas":"fichas","Prom/ficha":"prom"}[ord_mun]
        rows_mun = sorted(rows_mun, key=lambda x: -x.get(ord_key, 0))

        if rows_mun:
            max_val = rows_mun[0].get(ord_key, 1)
            for r in rows_mun[:15]:
                mn   = r.get("mun","")
                ap   = r.get("ap", 0)
                fich = r.get("fichas", 0)
                pr   = r.get("prom", 0)
                sec  = MUN_STATS.get(mn,{}).get("sector","—")
                tend = MUN_STATS.get(mn,{}).get("tend",0)
                val_disp = r.get(ord_key, 0)
                pct  = round(val_disp / max(max_val,1) * 100)
                tc   = "#0F6E56" if tend>0 else ("#991B1B" if tend<0 else "#94A3B8")
                tb   = f"▲{tend}%" if tend>0 else (f"▼{abs(tend)}%" if tend<0 else "→")
                est  = ("🟢" if pr>=27 else ("🔵" if pr>=22 else "🟡"))
                st.markdown(
                    f'<div style="margin-bottom:7px;padding:8px 10px;background:#fff;'
                    f'border:.5px solid #E2E8F0;border-radius:9px">'
                    f'<div style="display:flex;justify-content:space-between;'
                    f'font-size:11px;margin-bottom:4px">'
                    f'<span style="font-weight:600;color:#0F172A">{est} {mn}</span>'
                    f'<span style="display:flex;gap:8px;align-items:center">'
                    f'<span style="color:#64748B;font-size:10px">{sec[:14]}</span>'
                    f'<span style="color:{tc};font-weight:600;font-size:10px">{tb}</span>'
                    f'</span></div>'
                    f'<div style="height:6px;background:#F1F5F9;border-radius:3px;overflow:hidden;margin-bottom:4px">'
                    f'<div style="width:{pct}%;height:100%;background:#1E40AF;border-radius:3px"></div></div>'
                    f'<div style="display:flex;gap:10px;font-size:10px;color:#64748B">'
                    f'<span>📚 {fich} fichas</span>'
                    f'<span>👥 {ap:,} ap</span>'
                    f'<span>📈 {pr}/ficha</span>'
                    f'</div></div>',
                    unsafe_allow_html=True)
        else:
            st.info("Sin datos para el filtro seleccionado. Selecciona 'Todos' en el municipio global.", icon="ℹ️")

    with col_prog:
        st.markdown(f"**⭐ Programa más demandado por municipio — {nf_lbl}**")

        if nf == "Todos":
            # Para "Todos": mostrar top programa de cada municipio (cualquier nivel)
            prog_top_all = []
            for mun in sorted(MUN_STATS.keys()):
                progs_m = MUN_PROGS.get(mun, [])
                if progs_m:
                    top_p = progs_m[0]
                    prog_top_all.append({
                        "mun": mun, "prog": top_p["prog"],
                        "ap": top_p["ap"], "nivel": top_p["nivel"]
                    })
            prog_rows = sorted(prog_top_all, key=lambda x: -x["ap"])[:15]
        else:
            prog_rows = PROG_TOP_D.get(nf, [])[:15]

        # Filtrar por municipio global
        if g_mun != "Todos":
            prog_rows = [r for r in prog_rows if r.get("mun","") == g_mun]

        if prog_rows:
            max_ap = prog_rows[0].get("ap",1)
            for r in prog_rows:
                mn   = r.get("mun","")
                prog = r.get("prog","")
                ap   = r.get("ap",0)
                niv  = r.get("nivel","") if nf=="Todos" else nf
                pct  = round(ap/max(max_ap,1)*100)
                nc   = CN.get(niv, "#64748B")
                nl   = LN.get(niv, niv)[:12]
                st.markdown(
                    f'<div style="display:flex;align-items:center;gap:7px;'
                    f'padding:7px 10px;background:#fff;border:.5px solid #E2E8F0;'
                    f'border-radius:8px;margin-bottom:5px">'
                    f'<div style="flex:1;min-width:0">'
                    f'<div style="font-size:10px;font-weight:700;color:#0F172A;'
                    f'white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{mn}</div>'
                    f'<div style="font-size:10px;color:#64748B;'
                    f'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px">{prog[:42]}</div>'
                    f'<div style="height:3px;background:#F1F5F9;border-radius:2px;overflow:hidden;margin-top:4px">'
                    f'<div style="width:{pct}%;height:100%;background:{nc};border-radius:2px"></div></div>'
                    f'</div>'
                    f'<div style="text-align:right;flex-shrink:0">'
                    f'<div style="font-size:13px;font-weight:700;color:{nc}">{ap:,}</div>'
                    f'<div style="font-size:9px;color:#94A3B8">{nl}</div>'
                    f'</div></div>',
                    unsafe_allow_html=True)
        else:
            st.info("Sin datos para el filtro.", icon="ℹ️")

    # ── Tendencia anual por municipio ─────────────────────────────────────
    st.markdown("---")
    st.markdown("**📈 Tendencia anual — aprendices por municipio**")
    st.caption("Selecciona municipios específicos o filtra por sector para analizar la evolución.")

    # Construir datos de evolución desde TEND_ANUAL_D
    años_disp = sorted({
        a for v in TEND_ANUAL_D.values() for a in v.keys()
    })

    # Seleccionar municipios a mostrar
    muns_tend = [m for m in MUNICIPIOS
                 if g_mun == "Todos" or m == g_mun]
    if filt_sec != "Todos":
        muns_tend = [m for m in muns_tend
                     if MUN_STATS.get(m,{}).get("sector","") == filt_sec]

    # Tomar top 8 por aprendices totales
    muns_tend = sorted(muns_tend,
                       key=lambda m: MUN_STATS.get(m,{}).get("ap",0),
                       reverse=True)[:8]

    if muns_tend and años_disp:
        # Construir DataFrame
        rows_ev = []
        for mun in muns_tend:
            ta = TEND_ANUAL_D.get(mun, {})
            for a in años_disp:
                rows_ev.append({"Año": int(a), "Municipio": mun,
                                 "Aprendices": ta.get(str(a), ta.get(a, 0))})
        df_ev = pd.DataFrame(rows_ev)
        df_ev = df_ev[df_ev["Aprendices"] > 0]

        if not df_ev.empty:
            fig_ev = px.line(df_ev, x="Año", y="Aprendices", color="Municipio",
                markers=True,
                color_discrete_sequence=["#1E40AF","#0F6E56","#F59E0B","#DC2626",
                    "#7C3AED","#0891B2","#BE123C","#64748B"],
                labels={"Aprendices":"Aprendices","Año":"Año","Municipio":"Municipio"})
            fig_ev.update_layout(
                height=320,
                margin=dict(t=20,b=30,l=10,r=10),
                plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                legend=dict(orientation="h", y=-0.18, x=0, font=dict(size=10)),
                xaxis=dict(gridcolor="#F1F5F9", tickfont=dict(size=10), dtick=1),
                yaxis=dict(gridcolor="#F1F5F9", tickfont=dict(size=10)),
                hovermode="x unified")
            fig_ev.update_traces(line_width=2.5, marker_size=7)
            st.plotly_chart(fig_ev, use_container_width=True)
    else:
        st.info("Selecciona 'Todos' en el filtro de municipio para ver la tendencia.", icon="ℹ️")

    # ── Tabla resumen ─────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown("**Tabla de avances por municipio**")
    filas = []
    muns_tabla = [m for m in sorted(MUN_STATS.keys(),
                  key=lambda x: -MUN_STATS[x]["ap"])
                  if (g_mun == "Todos" or m == g_mun)
                  and (filt_sec == "Todos" or MUN_STATS[m]["sector"] == filt_sec)]
    for mn in muns_tabla:
        v  = MUN_STATS[mn]
        t  = v.get("tend",0)
        tb = f"▲ {t}%" if t>0 else (f"▼ {abs(t)}%" if t<0 else "→")
        es = "🟢 Alto" if v["prom"]>=27 else ("🔵 Normal" if v["prom"]>=22 else "🟡 Bajo")
        filas.append([mn, str(v["fichas"]), f'{v["ap"]:,}', str(v["prom"]),
                      v["sector"], tb, es])
    if filas:
        st.markdown(
            tabla_html(["Municipio","Fichas","Aprendices","Prom/f",
                        "Sector dominante","Tendencia","Impacto"], filas),
            unsafe_allow_html=True)
    else:
        st.markdown(
            tabla_html(["Municipio","Fichas","Aprendices","Prom/f",
                        "Sector dominante","Tendencia","Impacto"],
                       [[m, str(MUN_STATS[m]["fichas"]),
                         f'{MUN_STATS[m]["ap"]:,}',
                         str(MUN_STATS[m]["prom"]),
                         MUN_STATS[m]["sector"],
                         f'▲ {MUN_STATS[m].get("tend",0)}%' if MUN_STATS[m].get("tend",0)>0 else "→",
                         "🟢 Alto" if MUN_STATS[m]["prom"]>=27 else "🔵 Normal"]
                        for m in sorted(MUN_STATS.keys(),
                                        key=lambda x:-MUN_STATS[x]["ap"])[:22]]),
            unsafe_allow_html=True)
...
# =============================================================================
    ...
    st.markdown('<div class="sd">Guía completa SN Predict v3.2 — Occidente de Antioquia</div>', unsafe_allow_html=True)
...
""")
    ...
    with st.expander("⚙️ Consideraciones técnicas"):
        st.markdown("""
- **Datos:** PE04 Histórico Occidente de Antioquia (2020-2025) · 2.551 fichas · 22 municipios · 12 sectores.
...
# Footer
st.markdown(
    '<div class="foot">SN Predict v3.2 © 2026 · Occidente de Antioquia · '
